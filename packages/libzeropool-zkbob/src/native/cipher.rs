use crate::{
    constants::{self, ACCOUNT_HEAPLESS_SIZE, NOTE_HEAPLESS_SIZE, SHARED_SECRETS_HEAPLESS_SIZE},
    fawkes_crypto::{
...
use super::symmetric::{
    decrypt_chacha_constant_nonce, decrypt_xchacha, encrypt_chacha_constant_nonce, encrypt_xchacha,
    keccak256, Buffer,
};
...
pub fn encrypt<P: PoolParams>(
    ...
    let account_data = {
        ...
        let account_ciphertext =
            encrypt_chacha_constant_nonce(&account_key, &account.try_to_vec().unwrap());
    ...
    let shared_secret_data = {
        ...
        let text: Vec<u8> = core::iter::once(&account_data.0[..])
            .chain(notes_data.iter().map(|e| &e.1[..]))
            .collect::<Vec<_>>()
            .concat();
    ...
    (MessageEncryptionType::Symmetric.to_u16())
        .serialize(&mut res)
        .unwrap();
...
pub fn decrypt_out<P: PoolParams>(
    ...
    let shared_secret_ciphertext_size =
        nozero_items_num * constants::U256_SIZE + constants::POLY_1305_TAG_SIZE;

    let account_hash = Num::deserialize(&mut memo).ok()?;
    let note_hashes = buf_take(&mut memo, nozero_notes_num * num_size)?;

    let shared_secret_text = decrypt_shared_secrets::<P, SHARED_SECRETS_HEAPLESS_SIZE>(
        enc_type,
        eta,
        kappa,
        &mut memo,
        shared_secret_ciphertext_size,
        params,
    )?;
    ...
    let account_key =
        <[u8; constants::U256_SIZE]>::deserialize(&mut shared_secret_text_ptr).ok()?;
    let note_key = (0..nozero_notes_num)
...
fn decrypt_shared_secrets<P: PoolParams, const N: usize>(
    enc_type: MessageEncryptionType,
    eta: Num<P::Fr>,
    kappa: &[u8; 32],
    buf: &mut &[u8],
    size: usize,
    ...
    match enc_type {
        MessageEncryptionType::ECDH => {
            let a_p =
                EdwardsPoint::subgroup_decompress(Num::deserialize(buf).ok()?, params.jubjub())?;
...
fn shared_secrets_size(
    enc_type: MessageEncryptionType,
    num_size: usize,
    items_num: usize,
) -> Option<usize> {
    let shared_secret_ciphertext_size =
        items_num * constants::U256_SIZE + constants::POLY_1305_TAG_SIZE;
    match enc_type {
        MessageEncryptionType::ECDH => {
...
fn symcipher_decryption_keys_user_data<P: PoolParams>(
    eta: Num<P::Fr>,
    kappa: &[u8; 32],
    enc_type: MessageEncryptionType,
    mut memo: &[u8], //assumed this is the end of the memo only with the user data
    params: &P,
    ...
    if let Some(shared_secret_text) = shared_secret_text {
        ...
        let result = (0..nozero_items_num)
            .filter_map(|i| {
                ...
                match decrypt_user_data_no_validate(&data_keys[i], ciphertext, params) {
                    Some(_) => Some((i as u64, ciphertext.to_vec(), data_keys[i].to_vec())),
                    _ => None,
                }
            })
            .collect::<Vec<_>>();
        ...
        let user_data = (0..nozero_items_num)
            .filter_map(|i| {
                let a_pub = EdwardsPoint::subgroup_decompress(
                    Num::deserialize(&mut memo).ok()?,
                    params.jubjub(),
                ...
                match decrypt_user_data_no_validate(&key, ciphertext, params) {
                    Some(_) => Some((i as u64, ciphertext.to_vec(), key.to_vec())),
                    _ => None,
                }
            })
            .collect();
...
fn symcipher_account_notes<P: PoolParams>(
    eta: Num<P::Fr>,
    kappa: &[u8; 32],
    mut memo: &[u8],
    params: &P,
    ...
    if let Some(shared_secret_text) = shared_secret_text {
        ...
        let result = Some(account_tuple)
            .into_iter()
            .chain((0..nozero_notes_num).filter_map(|i| {
                buf_take(&mut memo, num_size)?;
...
pub fn symcipher_decryption_keys<P: PoolParams>(
    eta: Num<P::Fr>,
    kappa: &[u8; 32],
    mut memo: &[u8],
    params: &P,
...
pub fn decrypt_account<P: PoolParams>(
    symkey: &[u8],
    ciphertext: &[u8],
    hash: Num<P::Fr>,
    params: &P,
...
pub fn decrypt_account_no_validate<P: PoolParams>(
    symkey: &[u8],
    ciphertext: &[u8],
    _: &P,
) -> Option<Account<P::Fr>> {
...
pub fn decrypt_note<P: PoolParams>(
    symkey: &[u8],
    ciphertext: &[u8],
    hash: Num<P::Fr>,
    params: &P,
...
pub fn decrypt_note_no_validate<P: PoolParams>(
    symkey: &[u8],
    ciphertext: &[u8],
    _: &P,
) -> Option<Note<P::Fr>> {
...
pub fn decrypt_user_data_no_validate<P: PoolParams>(
    symkey: &[u8],
    ciphertext: &[u8],
    _: &P,
) -> Option<ExtraData<P::Fr>> {
    let plain = decrypt_chacha_constant_nonce::<NOTE_HEAPLESS_SIZE>(&symkey, ciphertext)?;
    let res = ExtraData::try_from_slice(plain.as_slice()).ok();

    return res;
}

/// Deprecated but still in use on the old pools
pub fn _encrypt_old_user_data<P: PoolParams>(
    entropy: &[u8],
    eta: Num<P::Fr>,
    user_data: &[ExtraData<P::Fr>],
    params: &P,
) -> Vec<u8> {
    let nozero_items_num = user_data.len() + 1;
    let mut sb = SeedboxChaCha20::new_with_salt(entropy);
    ...
    let user_data_arr = user_data
        ...
        .map(|e| {
            ...
            (a_pub.x, key, ciphertext_size as u32, ciphertext)
        })
        .collect::<Vec<_>>();
    ...
    let shared_secret_data = {
        ...
        let key = keccak256(&ecdh.x.try_to_vec().unwrap());
        let text: Vec<u8> = core::iter::once(&account_key[..])
            .chain(user_data_arr.iter().map(|e| &e.1[..]))
            .collect::<Vec<_>>()
            .concat();
    ...
    user_data_arr.iter().for_each(|nd| {
        nd.0.serialize(&mut res).unwrap();
        nd.2.serialize(&mut res).unwrap();
        res.extend(&nd.3);
    });
...
mod tests {
    ...
    use crate::native::cipher::{
        _encrypt_old_user_data, decrypt_account, decrypt_note, encrypt, symcipher_decryption_keys,
    };
    use crate::native::key::{derive_key_a, derive_key_eta, derive_key_kappa, derive_key_p_d};
    use crate::native::note::{ExtraData, Note};
    use crate::native::symmetric::{
        decrypt_chacha_constant_nonce, decrypt_xchacha, encrypt_chacha_constant_nonce,
        encrypt_xchacha,
    };
    use crate::{native::boundednum::BoundedNum, POOL_PARAMS};
    use fawkes_crypto::ff_uint::Num;
    use fawkes_crypto::rand::rngs::OsRng;
    use fawkes_crypto::{engines::bn256::Fr, rand::Rng};

    use super::{_encrypt_old, decrypt_in, decrypt_out, MessageEncryptionType};
    ...
    fn test_decrypt_in_out(
        ...
        let notes: Vec<Note<Fr>> = (0..notes_count as u64)
            .map(|_| {
    ...
    fn test_compliance(notes_count: u32, note_probability: f64, enc_type: MessageEncryptionType) {
        ...
        sender_restored.iter().for_each(|(index, chunk, key)| {
            if *index == 0 {
                ...
                let decrypt_note = decrypt_note(
                    key.as_slice(),
                    chunk.as_slice(),
                    orig_note.hash(params),
                    params,
                )
                .unwrap();
        ...
        let receiver_restored =
            symcipher_decryption_keys(eta2, &kappa2, encrypted.as_slice(), params).unwrap();
        ...
        let thirdparty_restored =
            symcipher_decryption_keys(eta3, &kappa3, encrypted.as_slice(), params).unwrap();

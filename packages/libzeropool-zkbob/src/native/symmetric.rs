...
use chacha20poly1305::{
    aead::AeadMutInPlace, ChaCha20Poly1305, Key, Nonce, XChaCha20Poly1305, XNonce,
};
...
pub(crate) fn decrypt_chacha_constant_nonce<const N: usize>(
    key: &[u8],
    ciphertext: &[u8],
) -> Option<Buffer<u8, N>> {
    assert!(key.len() == constants::U256_SIZE);
...
/// Decrypts message in place if `ciphertext.len()` is less or equals to N, otherwise allocates memory in heap.
pub(crate) fn decrypt_xchacha<const N: usize>(
    key: &[u8],
    nonce: &[u8],
    ciphertext: &[u8],
) -> Option<Buffer<u8, N>> {

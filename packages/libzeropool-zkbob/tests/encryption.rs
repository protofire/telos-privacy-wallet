...
use libzeropool_zkbob::native::{account::Account, cipher, key::derive_key_p_d, note::Note};
...
fn test_encryption() {
    ...
    let mut note: Vec<Note<Fr>> = (0..2)
        .map(|_| Note::sample(&mut rng, &*POOL_PARAMS))
        .collect();
    ...
    let ciphertext = cipher::encrypt(
        &(0..32).map(|_| rng.gen()).collect::<Vec<_>>(),
        &sender_kappa,
        account,
        &note,
        &*POOL_PARAMS,
    ...
    let (account_out, note_out, extra_data) = result_out.unwrap();
    assert!(
        note.len() == note_out.len()

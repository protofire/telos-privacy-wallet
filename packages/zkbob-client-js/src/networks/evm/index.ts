...
import { recoverTypedSignature, signTypedData, SignTypedDataVersion, personalSign, recoverPersonalSignature } from "@metamask/eth-sig-util";
...
export class EvmNetwork extends MultiRpcManager implements NetworkBackend, RpcManagerDelegate {
  ...
  private contractByType(type: ZkBobContractType): Contract {
    ...
    switch (type) {
      case ZkBobContractType.Pool:
        res = this.pool;
        break;
      case ZkBobContractType.DD:
        res = this.dd;
        break;
      case ZkBobContractType.Token:
        res = this.token;
        break;
      case ZkBobContractType.Accounting:
        res = this.accounting;
        break;
    }
  ...
  public async erc3009AuthState(tokenAddress: string, authorizer: string, nonce: bigint): Promise<bigint> {
    const res = await this.contractCallRetry(ZkBobContractType.Token, tokenAddress, "authorizationState", [
      authorizer,
  ...
  public async approveTokens(
    ...
    const receipt = await this.commonRpcRetry(
      async () => {
        return this.activeWeb3().eth.sendSignedTransaction(signedTx.rawTransaction ?? "");
      },
      "Unable to send approve tx",
      true
    ); // do not retry sending to avoid any side effects
  ...
  public async createCommitForcedExitTx(poolAddress: string, forcedExit: ForcedExitRequest): Promise<PreparedTransaction> {
    ...
    const encodedTx = await this.contractByType(ZkBobContractType.Pool)
      .methods[method](
        forcedExit.operator,
        forcedExit.to,
        ...
        [forcedExit.tx_proof.a, forcedExit.tx_proof.b, forcedExit.tx_proof.c].flat(2)
      )
      .encodeABI();
  ...
  public async committedForcedExit(poolAddress: string, nullifier: bigint): Promise<CommittedForcedExit | undefined> {
    ...
    const associatedEvents = await this.activeWeb3().eth.getPastLogs({
      address: poolAddress,
      topics: [[commitSignature, cancelSignature], addHexPrefix(nullifier.toString(16).padStart(64, "0"))],
      fromBlock: 0,
      toBlock: "latest",
    });
  ...
  public async executedForcedExit(poolAddress: string, nullifier: bigint): Promise<FinalizedForcedExit | undefined> {
    ...
    const associatedEvents = await this.activeWeb3().eth.getPastLogs({
      address: poolAddress,
      topics: [[executeSignature], null, addHexPrefix(nullifier.toString(16).padStart(64, "0"))],
      fromBlock: 0,
      toBlock: "latest",
    });

    if (associatedEvents.length > 0) {
      const decoded = this.activeWeb3().eth.abi.decodeLog(
        executeEventAbi.inputs ?? [],
        associatedEvents[0].data,
        associatedEvents[0].topics.slice(1)
      );
      return {
        nullifier: BigInt(decoded.nullifier),
        to: decoded.to,
  ...
  public async getTxDetails(index: number, poolTxHash: string, state: ZkBobState): Promise<PoolTxDetails | null> {
    try {
      ...
      if (transactionObj && transactionObj.blockNumber && transactionObj.input) {
        ...
        const block = await this.activeWeb3()
          .eth.getBlock(transactionObj.blockNumber)
          .catch(() => null);
  ...
  public async getBlockNumberFrom(rpcurl: string): Promise<number> {
    ...
    return this.commonRpcRetry(
      ...
      `[EvmNetwork]: Cannot get block number from ${rpcurl}`,
      true
    );
  }

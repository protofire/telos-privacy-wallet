...
use libzeropool::{
    ...
    native::{
        ...
        tx::{
            ...
            tx_sign, TransferPub, TransferSec, Tx,
        },
    },
};
...
use crate::{
    address::{format_address, parse_address, AddressFormat, AddressParseError},
    keys::{reduce_sk, Keys},
    merkle::Hash,
    random::CustomRng,
    ...
    P::Fr: 'static,
{
    ...
    pub fn is_own_address(&self, address: &str) -> bool {
        match parse_address::<P>(address, &self.params, self.pool_id) {
            Ok((d, p_d, _)) => self.is_derived_from_our_sk(d, p_d),
            Err(_) => false,
        }
    }

    pub fn is_derived_from_our_sk(
        &self,
        d: BoundedNum<P::Fr, { constants::DIVERSIFIER_SIZE_BITS }>,
        p_d: Num<P::Fr>,
    ) -> bool {
    ...
    pub fn create_tx(
        ...
        let in_account = in_account_optimistic.unwrap_or_else(|| {
            state
                .latest_account
                .unwrap_or_else(|| self.initial_account())
        });
        ...
        let delta_index = Num::from(delta_index.unwrap_or_else(|| {
            let next_by_optimistic_leaf = extra_state.new_leafs.last().map(|leafs| {
                (((leafs.0 + (leafs.1.len() as u64) - 1) >> constants::OUTPLUSONELOG) + 1)
    ...
    pub fn get_tx_input(&self, index: u64) -> Option<TransactionInputs<P::Fr>> {
        ...
        let input_acc = self
            .state
            .get_previous_account(index)
            .unwrap_or_else(|| (0, self.initial_account()));
        ...
        let inh = nullifier_intermediate_hash(
            input_acc.1.hash(params),
            self.keys.eta,
            index.into(),
            params,
        );
...
mod tests {
    ...
    fn test_create_tx_deposit_zero() {
        ...
        acc.create_tx(TxType::Deposit(op, vec![], BoundedNum::ZERO), None, None)
            .unwrap();
    }
    ...
    fn test_create_tx_deposit_one() {
        ...
        acc.create_tx(
            TxType::Deposit(op, vec![], BoundedNum::new(Num::ONE)),
            None,
            None,
        )
        .unwrap();
    ...
    fn test_create_tx_transfer_zero() {
        ...
        acc.create_tx(TxType::Transfer(op, vec![], vec![out]), None, None)
            .unwrap();
    }
    ...
    fn test_tx_inputs() {
        ...
        let mut rng = CustomRng;
        let state = State::init_test(POOL_PARAMS.clone());
        let mut user_account = UserAccount::new(Num::ZERO, 1, false, state, POOL_PARAMS.clone());
        ...
        (0..10).into_iter().for_each(|idx| {
            user_account
                .state
                .add_note(1024 + idx + 1, Note::sample(&mut rng, &params))
        });
    ...
    fn test_chain_specific_addresses() {
        let acc_polygon = UserAccount::new(
            Num::ZERO,
            0,
            false,
            State::init_test(POOL_PARAMS.clone()),
        ...
        let acc_sepolia = UserAccount::new(
            Num::ZERO,
            0,
            false,
            State::init_test(POOL_PARAMS.clone()),
        ...
        let acc_optimism = UserAccount::new(
            Num::ZERO,
            1,
            false,
            State::init_test(POOL_PARAMS.clone()),
        ...
        let acc_optimism_eth = UserAccount::new(
            Num::ZERO,
            2,
            false,
            State::init_test(POOL_PARAMS.clone()),
        ...
        assert!(acc_polygon.validate_universal_address(
            "PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kxBwa"
        )); // generic without a prefix
        assert!(acc_polygon.validate_universal_address(
            "zkbob:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kxBwa"
        )); // generic
        assert!(acc_polygon.validate_address(
            "zkbob_polygon:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kRF7i"
        ));
        assert!(!acc_polygon.validate_address(
            "zkbob_optimism:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5vHs5L"
        ));
        assert!(!acc_polygon.validate_address(
            "zkbob_polygon:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5vHs5L"
        ));

        assert!(acc_sepolia.validate_universal_address(
            "PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kxBwa"
        ));
        assert!(acc_sepolia.validate_universal_address(
            "zkbob:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kxBwa"
        ));
        assert!(acc_sepolia.validate_address(
            "zkbob_polygon:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kRF7i"
        )); // true due to the same pool_id
        assert!(!acc_sepolia.validate_address(
            "zkbob_optimism:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5vHs5L"
        ));
        assert!(!acc_sepolia.validate_address(
        ...
        assert!(acc_optimism.validate_universal_address(
            "PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kxBwa"
        ));
        assert!(acc_optimism.validate_universal_address(
            "zkbob:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kxBwa"
        ...
        assert!(acc_optimism.validate_address(
            "zkbob_polygon:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5vHs5L"
        )); // the lib cannot distinguish different address prefixes
        assert!(!acc_optimism.validate_address(
            "zkbob_optimism:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kRF7i"
        ));
        assert!(!acc_optimism.validate_address(
            "zkbob_optimism:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kR*&**7i"
        ));
        assert!(acc_optimism.validate_address(
            "zkbob_zkbober:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5vHs5L"
        ));
        assert!(acc_optimism.validate_address(
            "zkbob:optimism:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5vHs5L"
        ));
        assert!(!acc_optimism.validate_universal_address("zkbob:"));
        assert!(!acc_optimism.validate_address(":"));
        assert!(!acc_optimism.validate_address(""));

        assert!(acc_optimism_eth.validate_universal_address(
            "PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kxBwa"
        ));
        assert!(acc_optimism_eth.validate_universal_address(
            "zkbob:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kxBwa"
        ));
        assert!(acc_optimism_eth.validate_address(
            "zkbob_optimism_eth:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse1j9diw"
        ));
        assert!(!acc_optimism_eth.validate_address(
            "zkbob_polygon:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kRF7i"
        ));
        ...
        assert!(!acc_optimism_eth.validate_address(
            "zkbob_optimism_eth:PtfsqyJhA2yvmLtXBm55pkvFDX6XZrRMaib9F1GvwzmU8U4witUf8Jyse5kRF7i"
        ));
    }
    ...
    fn test_chain_specific_address_ownable() {
        ...
        let universal_addresses: Vec<String> = accs
            .iter()
            .map(|acc| acc.generate_universal_address())
            .collect();
